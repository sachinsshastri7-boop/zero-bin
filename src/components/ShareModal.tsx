...
import { Copy, Check, ShieldCheck, Flame, Clock, ExternalLink, QrCode } from "lucide-react";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
      <div className="w-full max-w-lg rounded-2xl border border-emerald-500/30 bg-zinc-950 p-6 shadow-2xl shadow-emerald-500/10">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-zinc-400">
                Key embedded in URL hash fragment `#`
              </p>
            </div>
          </div>
        ...
        <div className="mt-5 space-y-4">
          {showQr ? (
            <div className="flex flex-col items-center justify-center space-y-3 py-4 bg-zinc-900/40 rounded-xl border border-zinc-800">
              <div className="p-3 bg-white rounded-xl shadow-lg">
                <QRCodeSVG value={shareUrl} size={180} level="M" />
              </div>
              <p className="text-xs text-zinc-400 font-mono">
                Scan to open encrypted link on mobile
              </p>
            </div>
          ) : (
            <div>
